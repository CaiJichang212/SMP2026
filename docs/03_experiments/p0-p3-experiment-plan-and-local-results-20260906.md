...
P0–P3 的代码路径和安全退化在本地通过，且通过官方 `api_client.py` 的 P0 固定终态门禁已取得 `20/20` 可比较的远端结果、四种终态均零分数波动。此前的 `RemoteProtocolError` 来自受限网络路径，而非 API 载荷或端点格式。随后完成的 P1/P2 校准共有 `555/555` 个可比较会话，但留出结算器的图级归一化 MAE 为 `10.24%`，未达到预注册的 `≤5%` 门禁。因此，当前提交继续使用无 LLM 的 `B1` 游说基线；`B2`、`B3`、`B4` 和 `B5` 不可晋级，也不能以本地 mock/单元测试的结果宣称优于 B1。
...
| P2/P3 矩阵静态展开 | 596 个会话 | 清单可由 runner 生成；20 个 P0 + 576 个主会话 |
...
| P2 图留出校准 | 420 / 420 可比 | Spearman 0.9543 通过，但归一化 MAE 10.24% 超过 5%，拒绝冻结档案 |
...
2. **进入 P1 校准，而不是直接扩大主矩阵。** P0 已通过；下一步按预注册的响应表与图留出实验冻结 `CalibrationProfile`。任何协议失败、哈希差异或 >2% 波动都应重新打开 P0 门禁，而不是以历史记录补齐。
3. **把 P2 的“模型正确性”和“策略收益”分开。** 当前 420-session 图留出已经直接否决当前结算器：它能排序但误差过大。应优先修正 `path5`、`triangle`、`star5` 的结算近似，再重新预注册并采集新图族；不能用当前高 Spearman 掩盖 MAE 失败。
