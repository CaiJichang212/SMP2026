...
- 单个种子 LLM 调用次数上限为：初赛 120 次、复赛 250 次。早期 SDK 的 50 次为本地内测占位保护值；并列时还比较剩余精神力和 API 调用数。
- `local_test.py` 的 `global_setting.max_api_calls` 是可按自定义种子调整的本地回合保护，不改变正式评测上限。其 `budget >= 2.0` 条件表示预算低于 2 时只剩扫描可用、已无法改变网络状态。
...
这已满足“多角色 + 环境反馈 + 后续重规划”的基本结构，但每个回合最多会调用 2 次 LLM。若持续这样运行，初赛会在 60 回合、复赛会在 125 回合触及正式调用上限；仍应使用规则决定探索和候选，避免将调用额度消耗在机械步骤上。因此基线应当作为架构示范，而非可直接提交的高分策略。
...
| 指挥官 + 侦察/执行官每轮各调一次 LLM | 初赛约 60 回合、复赛约 125 回合会耗尽正式配额，且机械决策浪费调用 | 使用规则决定探索/干预和候选，LLM 只在少数难以区分的候选间选择；每步最多一次 |
...
- 每个种子 LLM 调用数不超过当前赛段上限（初赛 120、复赛 250），并留出安全余量；统计剩余精神力。
...
- [ ] 最终 ZIP 已在 CPython 3.9 与 `networkx==3.1` 下直接导入；使用的全部
  `nx.*` API 均存在于 3.1。实际规划/控制器也已执行，记录真实框架版本及
  未覆盖的 FAQ 依赖；临时空框架类的导入不能作为兼容验收。
- [ ] 优化包与上次原样 ZIP 做成对动作复放：记录有效策略、规划异常、提案覆盖率、
  最终选中提案/基线次数、执行失败与分数。导入通过和有分数不能替代策略生效证据。
- [ ] 同分或微小本地均值改善没有被换算成官方预计得分；历史官方最高分版本保留为
  对照，开发数据与确认数据、模型情景与独立图实例分别统计。
