...
- 用户要求避免实验占满服务器。新实验默认单 worker，同时只允许一个重型计算任务；子 Agent 不自行并行启动实验或完整测试。
- 整个实验进程树共享 `CPUQuota=75%`（最多 0.75 核），配合 `Nice=19`、`MemoryHigh=1G`；当前双核服务器约留出 62.5% 的 CPU 总容量。
- 使用受限任务组运行重型命令并保存检查点，不限制同机其他服务。操作方式见 `docs/04_plan/experiment-resource-policy.md`。

## 策略安全边界

- 仅调用公开环境 API。每个动作发送前校验预算、节点、边、沟通次数和 `prompt_id`；返回结果才可更新本地状态。
- Python 先生成并校验候选；LLM 只能排序这些候选，须受调用配额限制，并对超时、异常和非法输出确定性回退。
- 当前提交默认是已通过独立门禁与最终 ZIP 验证的 P11 公开话术学习策略，保留已验证的 P9 降级路径。其他 CMG、结构规划和自适应探索变体只有通过对应校准/场景门禁后才能启用，门禁失败必须失效关闭。
